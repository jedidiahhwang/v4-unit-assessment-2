...
foods.forEach((element, index, arr) => {
  element.calories = (4 * element.carbs) + (4 * element.protein) + (9 * element.fat);
})
...
let saleProducts = products.map ((element, index, arr) => {
  element.price = element.price - (.25 * element.price);
  return element;
})
...
const orderTotal = blueProducts.reduce ((acc, el, i, arr) => {
  return acc + el.price;
}, 0)
